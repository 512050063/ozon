...
├── api/                   # API 请求层 (23 个模块，Axios 封装)
├── store/                 # Pinia Store (authStore, selectionStore)
...
- **UI 组件库** (`components/ui/`)：21 个通用业务组件，统一 `App-` 前缀命名
...
|------|------|--------|
| Routes | 路由定义、参数校验 | 21 |
| Controllers | 请求处理、响应格式化 | 19 |
| Services | 业务逻辑、外部API调用 | 14 |
...
**1688 货源搜索**: Controller → alibabaService (1688 API OAuth) → 解析返回 → 入库 collection_items
