...
assert.match(drawer, /toDisplayImageUrl\(imgUrl\)/, 'similar products drawer should proxy each carousel image for display')
assert.match(drawer, /getProductImages[\s\S]*?slice\(0,\s*5\)/, 'similar products drawer should cap the default carousel images at 5')
assert.match(drawer, /similar-carousel-strip/, 'similar products drawer should keep the product image carousel')
assert.match(drawer, /similar-carousel-slide-img/, 'similar products drawer should render carousel slides')
assert.match(drawer, /startCarousel\(product\)/, 'similar products drawer should start switching images on hover')
assert.match(drawer, /stopCarousel\(product\)/, 'similar products drawer should stop switching images when hover ends')
