...
export async function getDashboardSummary(userId: number): Promise<DashboardSummary> {
  ...
  const [
    ...
    prisma.collectionItem.count({ where: { isProcessed: false } }),
  ...
  const actionItems: DashboardSummary['actionItems'] = [
    ...
    },
    {
      key: 'collection',
      label: '待入库采集',
      value: unprocessedCollectionCount,
      description: '1688 采集库中还未移入商品库的条目',
      level: unprocessedCollectionCount > 0 ? 'warning' : 'info',
    },
    {
      key: 'sync-failed',
      label: '失败同步',
